...
try:
    ...
    if all(close):
        ...
        if not close[0]:
            print(f'\n---  0/{pts} points. Returned incorrect value for a.')
        if not close[1]:
            print(f'\n---  0/{pts} points. Returned incorrect value for b.')
        if not close[2]:
            print(f'\n---  0/{pts} points. Returned incorrect value for c.')
        if not close[3]:
...
try:
    ...
    if all(close):
        exec_grade += pts
        print(f'\n--- {pts}/{pts} points. d1, d2, d3, grad_wrt_U, grad_wrt_V, grad_wrt_W are correct.')
    else:
        if not close[0]:
            print(f'\n---  0/{pts} points. Returned incorrect value for d3.')
        if not close[1]:
            print(f'\n---  0/{pts} points. Returned incorrect value for d2.')
        if not close[2]:
            print(f'\n---  0/{pts} points. Returned incorrect value for d1.')
        if not close[3]:
    ...
    print(f"\n--- 0/{pts} points. An exception was raised by 'backward_layer3', 'backward_layer2', 'backward_layer1', or 'gradient'.\n")
    print(ex)
...
try:
    ...
    if rmse_min > 20 and rmse_max < 60:
        exec_grade += pts
        print(f'\n--- {pts}/{pts} points. Range of RMSE results are what is expected: 20 < RMSE < 60')
    else:
        print(f'\n--0  0/{pts} points.  Range of RMSE resuls should be between 20 and 60.  Yours are from {rmse_min} to {rmse_max}')
